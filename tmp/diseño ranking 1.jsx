function RankingPage() {
  ...
  return (
    <div className="container mx-auto p-6 min-h-screen bg-gray-50 rounded-lg">
      ...
      {/* Encabezado de tabla */}
      <div className="grid grid-cols-5 font-semibold text-gray-700 border-b pb-2 mb-2 text-sm sm:text-base">
      ...
      <div className="space-y-3 mb-8">
        {jugadores.map((jugador, index) => (
          <div
            ...
            className={`grid grid-cols-5 items-center p-4 rounded-xl border ${getPositionStyle(index)} transition-all duration-300 hover:shadow-md`}
          >
            {/* Posición */}
            <span
              className={`font-bold text-center rounded-full w-8 h-8 flex items-center justify-center mx-auto ${
                index === 0
            ...
            {/* Jugador + Premium (alineado horizontalmente) */}
            ...
            {/* Reputación */}
            <span className={`text-right font-bold ${getReputationColor(jugador.reputacion)}`}>
              {jugador.reputacion}
            </span>
