  ...
  <div className="min-h-screen bg-gray-50 px-8 py-12">
    <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-12">
      ...
      <div className="bg-white w-full md:w-1/3 p-6 rounded-2xl shadow-md border border-gray-200">
        ...
        {/* Bienvenida */}
        <h2 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 animate-pulse mb-2">
          ¡Bienvenido, {user ? user.username : "Usuario"}!
        </h2>

        {/* Reputación */}
        {user && (
          <div className="flex items-center justify-between bg-gradient-to-r from-orange-100 to-yellow-100 border border-orange-200 rounded-xl px-4 py-3 mb-6 shadow-sm">
            <div>
        ...
        {(profileErrors || []).map((error, i) => (
          <div
            key={i}
            className="bg-red-500 p-2 text-white my-2 rounded text-center"
          >
            {error}
          </div>
        ))}
        {(profileSuccess || []).map((success, i) => (
          <div
            key={i}
            className="bg-green-500 p-2 text-white my-2 rounded text-center"
          >
            {success}
          </div>
        ))}
      ...
      <div className="flex-1">
        ...
        {partidosByUser && partidosByUser.length > 0 ? (
          ...
          <p className="text-gray-500 text-sm mt-4">
            No has creado ningún partido aún.
          </p>
        )}
      </div>
    </div>
